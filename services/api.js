...
const UserAPI = {
    ...
    async getUserByPhone(phoneNumber) {
        try {
            console.log('通过手机号查询用户:', phoneNumber);
            
            // 构建查询条件 - 注意这里使用正确的字段名
            const phoneField = FEISHU_FIELD_MAP.users.手机号 || 'mobile';
            const filter = `${phoneField} $eq '${phoneNumber}'`;
            
            console.log('查询条件:', filter);
            
            // 调用飞书API查询用户
            const result = await BitableAPI.getUsers(filter);
            ...
            return {
                ...
                openid: user.fields[FEISHU_FIELD_MAP.users.用户OpenID] || '',
                nickName: user.fields[FEISHU_FIELD_MAP.users.昵称] || '王主任',
                avatarUrl: user.fields[FEISHU_FIELD_MAP.users.头像URL] || '/images/icons/user.png',
                phoneNumber: user.fields[FEISHU_FIELD_MAP.users.手机号] || phoneNumber,
                department: '社区管理员', // 假设这个字段不在表中
                role: 'admin' // 假设这个字段不在表中
            };
        } catch (error) {
    ...
    async getUserPosts(userId) {
        try {
            // 构建查询条件
            const authorField = FEISHU_FIELD_MAP.timeline.作者ID || 'author_id';
            const filter = `${authorField} $eq '${userId}'`;
            ...
            return (result.items || []).map(item => ({
                ...
                content: item.fields.content || '',
                type: item.fields.type || '公告',
                createTime: item.fields.publish_time || new Date().toISOString(),
